...
namespace NodeGraph.DataModel {
	...
	public class NodeData {
		...
		public ConnectionPointData AddDefaultInputPoint() {
			var p = m_inputPoints.Find(v => v.Label == InstenceSettings.DEFAULT_INPUTPOINT_LABEL);
			if(null == p) {
				p = AddInputPoint(InstenceSettings.DEFAULT_INPUTPOINT_LABEL);
			}
			return p;
		}

		public ConnectionPointData AddDefaultOutputPoint() {
			var p = m_outputPoints.Find(v => v.Label == InstenceSettings.DEFAULT_OUTPUTPOINT_LABEL);
			if(null == p) {
				p = AddOutputPoint(InstenceSettings.DEFAULT_OUTPUTPOINT_LABEL);
			}
			return p;
		}
